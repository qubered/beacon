...
|---|---|---|---|---|
| I1 | A transport node emits `bytes`. Decoding is always a separate, explicit node. | `nodes/transport` port signatures; `flow/types.Check` refuses bytes→string | `flow/types` connection tests; a registry test asserting every transport's primary output is `bytes` | types ✅ / registry M1 |
| I2 | Every flow run terminates. Deadline, node budget and loop caps enforced — and **the deadline reaches the socket**. | `engine/runtime`; per-transport abort adapters | A slow-node deadline test; a per-transport test that the socket is destroyed, not just abandoned | M1 / M2 |
| I3 | A published flow version is immutable. Editing forks a draft. | `flow/graph`; database constraint | Store test: update of a published version is rejected | M1 |
...
| I7 | An agent's egress policy is authoritative on the agent. Core cannot silently widen it. | `agent/egress` | Test: a Core-proposed widening is not applied without local approval | M2 / M4 |
