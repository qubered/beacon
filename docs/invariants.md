...
| I6 | An agent keeps monitoring while disconnected, and backfills on reconnect. | `agent/scheduler` (local state, no Core round trip), `agent/spool` (durable, sheds captures before results), `core/ingest` | `TestSpool_ShedsCapturesBeforeResults`, `TestSpool_ShedsResultsOnlyWhenNoCapturesRemain`, `TestSpool_SurvivesRestart`, `TestSpool_PeekIsAtLeastOnce` — mutation-checked by inverting the shed order and by making Peek destructive | shedding + durability ✅ / link partition and backfill M4 |
...
| I10 | Scheduling lateness never compounds. | `agent/scheduler.Schedule.Catchup` — the next slot advances from the previous *scheduled* value, never from now; catch-up snaps forward and reports the gap | `TestNext_LatenessDoesNotCompound` (ten cycles overrunning by 28s stay on the grid), `TestNext_CatchUpSnapsForwardAndRecordsTheGap`, `TestTicker_MissedRunsAreReportedNotReplayed`; mutation-checked by advancing from now and by replaying the backlog | ✅ |
