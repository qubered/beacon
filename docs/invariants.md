...
|---|---|---|---|---|
| I1 | A transport node emits `bytes`. Decoding is always a separate, explicit node. | `nodes/transport` port signatures; `flow/types.Check` refuses bytes→string | `flow/types` connection tests; `byteops.decode` and `parse.regex_extract` exercise the bytes→string→record chain end to end (`test/acceptance`) | types + decode chain ✅ / transport nodes M2 |
| I2 | Every flow run terminates. Deadline, node budget and loop caps enforced — and **the deadline reaches the socket**. | `engine/runtime.Run` (deadline via `context.WithTimeout`, node budget checked before dispatch); per-transport abort adapters land with real sockets | `TestDeadline_TerminatesASlowRun`, `TestNodeBudget_AbortsWhenExceeded` — both race-clean across repeated runs | scheduler-level ✅ / socket-level M2 |
